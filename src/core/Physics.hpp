...
// -----------------------------------------------------------------------------
// High-level Physics manager (similar style to AssetManager/Renderer)
// Provides global ownership of Jolt resources and convenience methods
// for creating/updating/destroying bodies from systems/observers.
// -----------------------------------------------------------------------------
    ...
    public:
        ...
        // Initialize Jolt and internal resources. Safe to call multiple times.
        void init();

        // Shutdown and free Jolt resources.
        void shutdown();

        bool isInitialized() const { return physicsSystem != nullptr; }
        ...
        void setUserData(::JPH::BodyID id, flecs::entity_t ent);

        // Step simulation (uses internal job system & temp allocator)
    ...
    private:
        ...
        std::unique_ptr<FlecsContactListener> contactListener{nullptr};
        std::unique_ptr<FlecsCharacterContactListener> characterContactListener{nullptr};
    };

    // Global accessor (Meyers singleton)
