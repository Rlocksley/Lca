...
namespace Lca
{
    namespace Core
    {
        ...
        void createEye()
        {
            ...
            orthoEye = createBufferInterface
            (1,sizeof(OrthoEye), VK_BUFFER_USAGE_UNIFORM_BUFFER_BIT);
        }
        ...
        {
            OrthoEye _orthoEye;
            _orthoEye.position = glm::vec4(position, 1.0f);
            _orthoEye.direction = glm::vec4(direction, 0.0f);
            
            // Center the orthographic projection around (0, 0)
            // This makes it like a camera at (0, 0, 0) looking at (0, 0, -1)
            ...
            projection[2][2] *= -1.f;
            _orthoEye.projView = projection;
            _orthoEye.projView *= createViewMatrix(position, direction, glm::vec3(0.f,-1.f,0.f));
            memcpy(orthoEye.pMemory, &_orthoEye, sizeof(OrthoEye));
        }
    }
}